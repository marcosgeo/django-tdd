...
def test_get_single_movie(client, monkeypatch):
    payload = {"title": "The Big Lebowski", "genry": "commedy", "year": "1998"}
...
def test_get_all_movies(client, monkeypatch):
    payload = [
        ...
        {"title": "No Country for Old Man", "genre": "thriller", "year": 2007},
    ]
...
def test_update_movie_incorrect_id(client, monkeypatch):
    payload = {"title": "Rock", "genre": "drama", "year": "1982"}
    ...
    resp = client.put("/api/movies/99/", payload, content_type="application/json")
    assert resp.status_code == 404
